...
static struct astfb_info *dinfo;
static unsigned int g_init_fb;
...
static int set_dac_src_disp_out(struct astfb_dispsrc* src)
{
	...
	case DISPSRC_FB:
		if(g_init_fb == 0x0)
		{
			astfb_init_hw(dinfo->dev, dinfo);
			ast_fb_set_mode(&info->var, dinfo);
			g_init_fb=0xFF;
		}
...
int astfb_init_hw(struct device *dev, struct astfb_info *dinfo)
{
	...
	reg = ioread32((void * __iomem)(SCU_CLK_SELECT_REG)); printk("fb, 2, init, clk_sel: %X\n", reg);/* Fortify 	  [unused varibale]:: False Positive */
        /* Reason for False Positive – using for hardware reg configuration*/
...
void ast_fd_wait_2d_engine_idle(struct astfb_info *dinfo)
{
	...
	do {
		...
		reg2 = ioread32(dinfo->io_2d + 0x4C) & 0xFFFC0000;;/* Fortify[unused varibale]:: False Positive */
        /* Reason for False Positive – using for hardware reg configuration*/
		reg2 = ioread32(dinfo->io_2d + 0x4C) & 0xFFFC0000;;/* Fortify[unused varibale]:: False Positive */
        /* Reason for False Positive – using for hardware reg configuration*/
		reg2 = ioread32(dinfo->io_2d + 0x4C) & 0xFFFC0000;;/* Fortify[unused varibale]:: False Positive */
...
static int astfb_set_fix(struct fb_info *info)
{
	...
	strncpy(fix->id, dinfo->name,sizeof(fix->id));
	fix->smem_start = dinfo->frame_buf_phys;
	fix->smem_len = dinfo->frame_buf_sz;
	fix->type = FB_TYPE_PACKED_PIXELS;
	fix->type_aux = 0;
	fix->visual = (uint32_t)(dinfo->bpp == 8) ? FB_VISUAL_PSEUDOCOLOR : FB_VISUAL_TRUECOLOR;/* Fortify [Type Mismatch]:: False Positive */
	 /* Reason for False Positive - Already proper type casting taken care*/
	fix->xpanstep = 0;
	fix->ypanstep = ypan ? 1 : 0;
	fix->ywrapstep = 0;
	fix->line_length = (uint32_t)dinfo->pitch;
...
static int astfb_ioctl(struct fb_info *info, unsigned int cmd, unsigned long arg)
{
	...
	case AMIFB_GET_DFBINFO:
		...
		dfbinfo.ul2DMode = (unsigned long)dinfo->use_2d_engine;
...
static int astfb_set_fbinfo(struct astfb_info *dinfo)
{
	...
	{
		printk("Already in FB mode, so skipping set default mode\n");
	}
...
static int astfb_drv_probe(struct device *dev)
{
	...
	dinfo->info = info;
	dinfo->dev = dev;
	strncpy(dinfo->name,"astfb",sizeof(dinfo->name));
...
/*	if (astfb_init_hw(dev, dinfo)) {
	...
	if(dispsrc.val != DISPSRC_FB)
	{
		astfb_init_hw(dev, dinfo);
		g_init_fb=0xFF;
	}
	else
	{
		printk("Already in Fb mode, so skipping hw init\n");
	}
