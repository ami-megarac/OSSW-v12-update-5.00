...
#if defined(SOC_AST2500) || defined(CONFIG_SOC_AST2500)
...
int ast_fb_get_mode_info(struct fb_var_screeninfo *var, struct astfb_info *dinfo, struct ast_fb_mode_info_t *mode_info)
{
	...
	uint32_t ulHT, ulVT;/* Fortify [unused varibale]:: False Positive */
    /* Reason for False Positive – using for frame buffer configuration*/
	uint32_t ulHBorder, ulVBorder;

	ulHT = var->xres + var->left_margin + var->right_margin + var->hsync_len;/* Fortify [unused varibale]:: False Positive */
    /* Reason for False Positive – using for frame buffer configuration*/
	ulVT = var->yres + var->upper_margin + var->lower_margin + var->vsync_len;/* Fortify [unused varibale]:: False Positive */
    /* Reason for False Positive – using for frame buffer configuration*/
	...
	/* Update var info */
	ulHBorder = (uint32_t)(mode_info->flags & MODE_FLAG_H_BORDER) ? 1 : 0;;/* Fortify [unused varibale]::             False Positive */
        /* Reason for False Positive – using for frame buffer configuration*/
	ulVBorder = (uint32_t)(mode_info->flags & MODE_FLAG_V_BORDER) ? 1 : 0;/* Fortify [unused varibale]::              False Positive */
        /* Reason for False Positive – using for frame buffer configuration*/
...
void astfb_set_mode_reg(struct astfb_info *dinfo, struct ast_fb_mode_info_t *mode_info)
{
	...
	uint32_t horiz_border_end;/* Fortify [unused varibale]:: False Positive */
    /* Reason for False Positive – using for frame buffer configuration*/
	...
	uint32_t verti_border_end;/* Fortify [unused varibale]:: False Positive */
        /* Reason for False Positive – using for frame buffer configuration*/
	uint32_t verti_retrace_start;
	uint32_t verti_retrace_end;

	horiz_total = mode_info->horiz_total;
	horiz_display_end = mode_info->horiz_display_end;/* Fortify [unused varibale]:: False Positive */
       /* Reason for False Positive – using for frame buffer configuration*/
	horiz_border_start = (mode_info->flags & MODE_FLAG_H_BORDER) ? (horiz_display_end + 1) : horiz_display_end;
	horiz_border_end = (mode_info->flags & MODE_FLAG_H_BORDER) ? (horiz_total - 1) : horiz_total;/* Fortify [u	  nused varibale]:: False Positive */
        /* Reason for False Positive – using for frame buffer configuration*/
	...
	verti_display_end = mode_info->verti_display_end;/* Fortify [unused varibale]:: False Positive */
        /* Reason for False Positive – using for frame buffer configuration*/
	verti_border_start = (mode_info->flags & MODE_FLAG_V_BORDER) ? (verti_display_end + 1) : verti_display_end;
	verti_border_end = (mode_info->flags & MODE_FLAG_V_BORDER) ? (verti_total - 1) : verti_total;/* Fortify [u        nused varibale]:: False Positive */
        /* Reason for False Positive – using for frame buffer configuration*/
...
#if defined(SOC_AST2500) || defined(CONFIG_SOC_AST2500)
	iowrite32(0xe50610, dinfo->io + AST_GRAPHICS_THRES);
#else
