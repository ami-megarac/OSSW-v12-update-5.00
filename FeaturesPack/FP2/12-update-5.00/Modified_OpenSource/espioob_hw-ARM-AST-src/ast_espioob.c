...
int ast_espi_init(void)
{
    ...
    *(volatile u32 *)(IO_ADDRESS(AST_SCU_REG_BASE)) = 0;            //Lock SCU register
    ...
    espioob_irq = GetIrqFromDT("ami_espi", AST_ESPI_IRQ);
    ret = request_irq(espioob_irq, ast_espioob_handler, IRQF_SHARED, AST_ESPIOOB_DRIVER_NAME, ast_espioob_virt_base + AST_ESPI_OOB_CHAN_ID);
    if (ret) {
...
void ast_espi_exit(void)
{
    ...
    free_irq(espioob_irq, ast_espioob_virt_base + AST_ESPI_OOB_CHAN_ID);
