...
const struct DEVICE_INFO device_list[]={
  ...
  {.idcode =LATTICE_CPLD_LCMXO2_7000HE_IDCODE,	.name=LATTICE_CPLD_LCMXO2_7000HE_DEVNAME, .func = device_MACHOX2, .support_loop_func = 1},
...
#define AST_JTAG_BUFFER_SIZE 0x180000
	memset (JTAG_data_buffer, 0, AST_JTAG_BUFFER_SIZE);
    ...
    case DeviceFamily_Run_NewCode:{
      ...
      *buf = device_data_check(get_jtag_read_buffer(), 0, JTAG_device_information.Device_All_Bits_Length);
      ...
      SDR(32, &TDI,&TDO);
        ...
        if((TDO& 0x00003000) != 0) 
	    {
	       printk("Check Erase status fail![%x]\n",(TDO& 0x00003000));
      	   ret=-1;
	    }
		//-----------------------------------------------                
        else{
...
uint32_t load_device_info(unsigned long id_code){
  ...
  const JTAG_DEVICE_INFO MachXO_information[]=
  {
	...
	{ //LATTICE_CPLD_LCMXO2_7000HE_DEVNAME: 
      .Device_Name    =               LATTICE_CPLD_LCMXO2_7000HE_DEVNAME,
      .Device_ID      =               LATTICE_CPLD_LCMXO2_7000HE_IDCODE,
      .Device_Column_Length   =       LATTICE_COLS_LCMXO2_7000HE_LENGTH,
      .Device_Row_Length      =       LATTICE_ROWS_LCMXO2_7000HE_LENGTH,
      .Device_All_Bits_Length =       LATTICE_BITS_LCMXO2_7000HE_LENGTH,
      .Device_Bscan_Length    =       LATTICE_BSCAN_LCMXO2_7000HE_LENGTH,
      .Device_Fuses_Length    =       LATTICE_FUSES_LCMXO2_7000HE_LENGTH
    }, 
