...
INIHandler*  iniparser_loaddef(const char * def_ininame, const char * ininame)
{
    ...
    char        sec[ASCIILINESZ+1] = {0};
    char        key[ASCIILINESZ+1] = {0};
    char        val[ASCIILINESZ+1] = {0};
    char    *   where ;
    FILE    *   ini = NULL ;
    FILE    *   def_ini = NULL ;
    ...
    {
	printf("Error in opening files %s and %s\n",def_ininame,ininame);    
        return NULL;
    }
...
void iniparser_dump_ini(INIHandler *handler, FILE *file)
{
    ...
    char    keym[ASCIILINESZ+1] = {0};
    int     nsec = 0 ;
    char *  secname = NULL , *saveptr = NULL;
    int     seclen = 0;
    NVP_T **hashTable = NULL;
    NVP_T *nvp_ptr = NULL;
...
{
    FILE *fp;
    char bkp_filename[MAX_CONF_STR_LENGTH] = {0};
    ...
    /* Reason for False Positive - not accepting user inputs and to strings bkp_filename and filename, updated in source and not using invalid data while using the file */
