...
ProcessGetStatus(uint8 DevNo,DEVICE_REQUEST *pRequest)
{
	...
	if (REQ_RECIP_MASK(pRequest->bmRequestType) == REQ_ENDPOINT)
	{
		...
		if (UsbGetEpStatus(DevNo,((uint8)pRequest->wIndex & 0x7F),
			...
			Status[0] = (Stall)?0x01:0x00; /* Fortify [Type Mismatch: Signed to Unsigned]:: False Positive *//* Reason for False Positive - Conditional operator return 0 or 1, no any negative values. */
	}
