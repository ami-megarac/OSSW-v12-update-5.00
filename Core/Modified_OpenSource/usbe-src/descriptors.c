...
ConvertAsciizToUniCode(char *Str, uint16 *UniCodeLen)
{
	...
	*UniCodeLen = (StrLen+1)*2 + 2;
	...
	while (*Str)
	{
		UniCodeStr[i] 	= (uint8)*Str; /* Fortify [Type Mismatch: Signed to Unsigned]:: False Positive *//* Reason for False Positive - Casting for fortify [Type Mismatch: Signed to Unsigned].*/
		UniCodeStr[i+1] = 0;
		i+=2;
		Str++;
	}
								...
								char *ManufStr, char*ProdStr, int IsThisLunDev)
{
	...
	if (IsThisLunDev)
	{
		...
		Str = ConvertAsciizToUniCode("serial",&StrLen); /* Fortify [Dead Code]:: False Positive *//* Reason for False Positive - This statement is preserved for further use. */
...
AddOsStringDesc(uint8 DevNo,uint8 VendorCode,char *VendSigStr)
{
    ...
    while(*VendSigStr)
	{
		OsStrDesc->qwSignature[i] 	= (uint8)*VendSigStr; /* Fortify [Type Mismatch: Signed to Unsigned]:: False Positive *//* Reason for False Positive - Casting for fortify [Type Mismatch: Signed to Unsigned].*/
...
AddExtPropertiesDesc(char *CustomPropertyStr)
{
	...
	{
		ExtPropDesc->bPropertyName[i] 	= (uint8)*CustomPropertyStr; /* Fortify [Type Mismatch: Signed to Unsigned]:: False Positive *//* Reason for False Positive - Casting for fortify [Type Mismatch: Signed to Unsigned].*/
