			...
			char *buffer, size_t buflen, int *errnop)
{
	...
	if(strlen(defshell)!=0)
	{
		...
		/* Reason for False Positive - In macro expanssion using the destination size to fill the destination and also comparing the length of source with destination before writing into the destination and checking the buffer overflow condition for the same */
	}
