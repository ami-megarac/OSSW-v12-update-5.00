...
{
       0,   124,   124,   124,   126,   156,   166,   171,   176,   176,
     176,   181,   197,   213,   229,   245,   245,   245,   245
};
#endif
...
yyreduce:
  ...
  switch (yyn)
    {
        case 4:
#line 127 "slp_filter_y.y"
    { 
    if(NULL != (yyval.filter_filter = SLPAllocFilter(yyvsp[-2].filter_int)))
    {
        ...
        {
            SLPFreeFilter(yyval.filter_filter) ; yyval.filter_filter = NULL ;
        }
    }
}
    break;
...
#line 177 "slp_filter_y.y"
  ...
  case 11:
#line 182 "slp_filter_y.y"
    {      /* presence test binds to single operand */
    if(NULL != (yyval.filter_filter = SLPAllocFilter(expr_present)))
    {
        SLPAttrList *attr = SLPAllocAttr(yyvsp[-1].filter_string, string, "*", (int)strlen("*") + 1);
        if(attr != NULL)
        {
  ...
  case 12:
#line 198 "slp_filter_y.y"
    {  /* must be an int or a bool */
    /* remember to touch up the token values to match the enum in SLP.h */
    if(NULL != (yyval.filter_filter = SLPAllocFilter(yyvsp[-1].filter_int)))
    {
        SLPAttrList *attr = SLPAllocAttr(yyvsp[-2].filter_string, integer, &(yyvsp[0].filter_int), sizeof(yyvsp[0].filter_int));
        if(attr != NULL)
        {
           SLP_INSERT(attr, &(yyval.filter_filter->attrs));
        }
  ...
  case 13:
#line 214 "slp_filter_y.y"
    {  /* must be an int or a bool */
    /* remember to touch up the token values to match the enum in SLP.h */
    if(NULL != (yyval.filter_filter = SLPAllocFilter(yyvsp[-1].filter_int)))
    {
        SLPAttrList *attr = SLPAllocAttr(yyvsp[-2].filter_string, boolean, &(yyvsp[0].filter_int), sizeof(yyvsp[0].filter_int));
        if(attr != NULL)
        {
           SLP_INSERT(attr, &(yyval.filter_filter->attrs));
        }
  ...
  case 14:
#line 230 "slp_filter_y.y"
    {   /* both operands are strings */
    if(NULL != (yyval.filter_filter = SLPAllocFilter(yyvsp[-1].filter_int)))
    {
        SLPAttrList *attr = SLPAllocAttr(yyvsp[-2].filter_string, string, yyvsp[0].filter_string, (int)strlen(yyvsp[0].filter_string) + 1 );
        if(attr != NULL)
        {
           SLP_INSERT(attr, &(yyval.filter_filter->attrs));
        }
