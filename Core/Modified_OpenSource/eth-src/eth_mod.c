...
static int is_hostmac_conf_file_exist(const char *filepath)
{
...
static int create_hostmac_conf_file(const char *filepath)
{
	struct file	*filp = NULL;
	...
	TINFO("Created new host mac conf\n");
	if (filp)
		filp_close(filp, NULL);
...
static int write_hostmac_conf_file(const char *filepath, const uint8 *host_mac)
{
	struct file	*filp = NULL;
	loff_t file_offset;
	unsigned int amount = ETH_ALEN;
	ssize_t nwritten = 0;
	 ...
	 */
	nwritten = kernel_write(filp, host_mac, amount,
				file_offset);
	...
	if (0 > nwritten)
	{
		TCRIT("unable to write host mac to file: %02X\n", nwritten);
		return -1;
	}
	else if (nwritten < amount) {
		TCRIT("unable to write whole host mac to file: %02X\n", nwritten);
		return -1;
	}
...
static int read_hostmac_conf_file(const char *filepath, uint8 *buf)
{
	struct file	*filp = NULL;
	loff_t file_offset;
	unsigned int amount = ETH_ALEN;
	 ...
	 * Backward compatibility for kernel 3.14.17 and previous version.
	 */
	nread = kernel_read(filp, file_offset, buf,
					amount);
#elif (LINUX_VERSION_CODE >= KERNEL_VERSION(5,2,8))
	/**
	 * For kernel 5.2.8 and later.
	 */
	nread = kernel_read(filp, buf, amount,
				&file_offset);
#endif // end #if (LINUX_VERSION_CODE <= KERNEL_VERSION(3,14,17))
	if (nread < 0) {
		TCRIT("unable to read host mac from file: %02X\n", nread);
		return -1;
	}
	else if (nread < amount) {
		TCRIT("unable to read whole host mac from file: %02X\n", nread);
	}
...
init_eth_device_module (void)
{
	 ...
	 * 		2. The conf file store the host mac as binary not readable string.
	 * 		3. BMC eth module reads the conf file to get host mac for every AC power cycle.
	 */
	if (!is_hostmac_conf_file_exist(CONF_FILE_HOST_MAC_ADDRESS))
	{	// generate a random mac address for host once only and store it into conf file for future use.
		random_ether_addr(hostmac);
		create_hostmac_conf_file(CONF_FILE_HOST_MAC_ADDRESS);
		write_hostmac_conf_file(CONF_FILE_HOST_MAC_ADDRESS, hostmac);
		read_hostmac_conf_file(CONF_FILE_HOST_MAC_ADDRESS, temp_hostmac);
	...
	{	// read host mac from conf file.
		if (read_hostmac_conf_file(CONF_FILE_HOST_MAC_ADDRESS, hostmac))
		{
			TINFO("read exist:\n");
			dump_buff(hostmac, ETH_ALEN);
			// reassign host mac...
