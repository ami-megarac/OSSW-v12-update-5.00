				...
				unsigned long SectorSize, unsigned char* buf,FLASH_INFO* flinfo)
{
	...
	int fmh_found = 0;
	size_t NumBytesRead;
        long unsigned int junk_count = 0; 
	...
	if( flinfo->read_flash(ModuleOffset,Size,&NumBytesRead,buf) != 0)
	{
      ...
      // FMH module is bounded to blocksize , hence having junk char in un-filled block size
      // (related to FMH_MODULE_SIZE_FIX in git-spx). Truncating the buf till valid char. 
      for (i =0; i < Size; i++)
      {
          if(buf[i] > 127 )  //Identify junk char Ascii 0 - 127
          {
	     junk_count++;
	     if((junk_count >= 1) && (buf[i] == buf[i-1]))  // Checks if two junks characters are read consecutively.
	         break;
          }
      }
      buf[i-1] = '\0';
...
/* We don't support seeked reads. read should start from 0 */
	...
	/* FMH Firmware Info can be upto one erase block. So allocate */
	fmhinfo = kmalloc(CONFIG_SPX_FEATURE_GLOBAL_ERASE_BLOCK_SIZE,GFP_KERNEL);
