...
FormiUsbScsiPacket(IUSB_SCSI_PACKET *iUsbScsiPkt,uint8 DevNo, uint8 IfNum,BOT_IF_DATA *MassData,uint8 LunNo)
{
    ...
    memcpy(&(iUsbScsiPkt->CommandPkt),&(cbw->CBWCB), cbw->bCBWCBLength); /* Fix for Fortify [Buffer Overflow]:: False Positive */
    /**
     * The size of structure CommandPkt is 12 bytes, 
     * not enough to store max size of CBW data(16 bytes).
     * The chage of structure declaration may impact many behavior.
     * Add warning message for copying cbwcb to check if issue occurred.
     */
    if(sizeof (iUsbScsiPkt->CommandPkt) < cbw->bCBWCBLength)
    {
		printk("!!! COPY overflow with CBWCB, %02Xh < %02Xh, please report the warning to maintainer!!!\n", sizeof(iUsbScsiPkt->CommandPkt), cbw->bCBWCBLength);
	}
