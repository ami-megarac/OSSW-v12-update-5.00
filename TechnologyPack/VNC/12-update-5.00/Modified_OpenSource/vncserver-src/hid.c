...
void CloseUSBDevice(void) {
	if (gUsbDevice > 0) {
		...
		close(gUsbDevice);
		if( remove(IUSB_KEY_FILE) < 0 ) { // Replaced system() with equivalent C API for fixing Fortify [Command Injection]
...
int GetIUSBLEDPkt(u8 *LEDstatus, int no_wait)
{
	...
	// If memset() were to fail due to compiler optimization, led sync functionality would've been broken
	memset(pBuffer, 0, sizeof(pBuffer) ); /* Fortify [Insecure Compiler Optimization]:: False Positive */
...
u8 CalculateModulo100(u8 *Buff, int BuffLen) {
	...
	for (i = 0; i < BuffLen; i++)
		ChkSum += Buff[i];
	return (~ChkSum + 1); /*Fortify [Type Mismatch: Signed to Unsigned]:: False Positive */ /*2's compliment calculation changed from (-ChkSum) to (~ChkSum + 1) - Reason*/ 
}
...
int mousePkt(int buttonMask, int x, int y, int maxx, int maxy) {
	...
	mouseRep.Scroll = 0;
	memcpy(&pHIDPacket->Data, &mouseRep, MAX_MOUSE_REPORT_SIZE); /* Fortify [Buffer Overflow]:: False Positive */
...
void SendKeyEvent(uint32 key, int8 down){
	...
	if (ConvertRFBtoUSBKey(KeybdInfo, down, key) == 0){
		memcpy(&prevkey, KeybdInfo, sizeof(prevkey)); /* Fortify [Out-of-Bounds Read]:: False Positive */
	}
