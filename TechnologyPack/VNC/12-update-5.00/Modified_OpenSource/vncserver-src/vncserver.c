...
int text_frame = -1;
#define	FRAME_TYPE_ASCII_ONLY		0
#define	FRAME_TYPE_ASCII_ATTR		1
#define	FRAME_TYPE_ASCII_ATTR_FONT	2
#define	BIT2 (1<<2)
#define	BIT3 (1<<3)
#define	BIT4 (1<<4)
static rfbFontDataPtr curFont = NULL;
rfbPixel color[16]={ /*Black*/		0x000000,
				/*blue*/			0x0000AA,
				/*green*/			0x00AA00,
				/*cyan*/			0x00AAAA,
				/*red*/				0xAA0000,
...
static void clientgone(rfbClientPtr cl) {
	...
	if ( cl->clientData != NULL ) {
		...
		if((active_sock_list == NULL) && (actvSessCount > 0)){ // Fix for Fortify [Integer Overflow]
	...
	if(actvSessCount == 0)
	{	// vnc_max_session, sessionInfo values are initialized upon receiving RECV_VNC_CONF command in pipe
...
{	// Followed libvncserver code spec. Check rfbProcessEvents()
	if(usec<0)
		usec = screen->deferUpdateTime*1000; /* Fortify [Dead Code]:: False Positive */
...
{
	double timedout; // Fix for Fortify [Poor Style: Redundant Initialization]
	time_t update_time;
	...
	while(cl)
	{
		timedout = 1;
		if(cl->sock != -1){
			update_time = getLastPacketTime(cl->sock); // Fix for Fortify [Poor Style: Value Never Read]
...
#endif // Initialized on linking libvideo_vnc
...
void fbUnmap(void) {
	if(munmap(fb_info.fbmem, fb_info.fix_screen_info.smem_len) != 0)
		printf("[%s:%d] munmap failed : %d\n", __FUNCTION__, __LINE__, errno);
}
...
void adoptResolutionChange(rfbClientPtr cl) {
	...
	/* intimate active clients */ // Allocated memory is released upon calling rfbReleaseClientIterator()
...
#endif
		/* Checking macro return value is risky because these definitions changes based on compilation flags */
		LOCK(cl->updateMutex);
...
void onStartStopVnc(int mode) {
	struct stat StatBuf; /* Fortify [Poor Style: Variable Never Used]:: False Positive */

	if (mode == STOP){
		if( DisableAutoSwitch == 1 ) { // Will be true if respective hidden feature is enabled in P.CONFIG
			if(stat(SET_CMD_IN_PROGRESS, &StatBuf) == 0) { /* Fortify [Dead Code]:: False Positive */
...
void* updateVideo()
{
	pthread_t self; /* Fortify [Poor Style: Variable Never Used]:: False Positive */
		...
		while(1){
			...
			if(powerCons == 1){
				...
				if( (VIDEO_NO_CHANGE == capture) || (VIDEO_NO_SIGNAL== capture) )
				{
					...
					if(bCounter >= 10 )
					{
						select_sleep(0, 100000); // Fix for Fortify [Obsolete]
					}
				}
				#if defined(SOC_AST2400) || defined(SOC_AST2500)
...
#if defined(SOC_PILOT_III) || defined(SOC_PILOT_IV)
						if ((TextFlag & BIT2) > 0) {
							text_frame = FRAME_TYPE_ASCII_ONLY;
							if ((TextFlag & BIT3) > 0) {
								text_frame = FRAME_TYPE_ASCII_ATTR;
								if ((TextFlag & BIT4) > 0) {
									text_frame = FRAME_TYPE_ASCII_ATTR_FONT;
								}
							}
						}
#endif
				...
				{
					select_sleep(0, 100); // Fix for Fortify [Obsolete]
				}
#endif
			...
			while(cl) {
				...
				if ( ( ( g_kvm_client_state != VNC ) && ( cl->sock != -1 ) ) ||
				 ( (cl->sock < 0 ) && (cl->clientData != NULL)) || (TRUE == cleanUp)) {
					...
					cleanUp = FALSE; /* Fortify [Poor Style: Value Never Read]:: False Positive */
				}
				cl=rfbClientIteratorNext(i);
			}
...
void showBlankScreen(char *message,int xPos,int yPos) {
	...
	if(rfbScreen->frameBuffer != emptyScreen){
		...
		if( maxx != VNC_MIN_RESX || maxy != VNC_MIN_RESY ) {
			if(emptyScreen != NULL) {
				free(emptyScreen); // maxx, maxy, bpp values are updated properly on full screen change
				emptyScreen= (char*)malloc(maxx*maxy*bpp); /* Fortify [Integer Overflow]:: False Positive */ /* condition to check integer overflow of maxa and maxy is present - Reason*/
				emptyScreenSize = maxx*maxy*bpp;
			}
		}
		if(emptyScreen != NULL) // Fix for Fortify [Missing Check against Null]
		{
			rfbScreen->frameBuffer = emptyScreen;
			adoptResolutionChange(pCl);
		}
	...
	if((emptyScreen != NULL) && (emptyScreenSize > 0))	// Fix for Fortify [Missing Check against Null], Values are initialized inside setupFb()
	...
	if(message != NULL && (message[0] != '\0')) {
		...
		{
#if defined(SOC_PILOT_III) || defined(SOC_PILOT_IV)		
			curFont = rfbLoadConsoleFont(FONT_FILE_WITH_ALT_ASCII_VALUE);
			if( curFont == NULL ){
				printf("\nError: unable to load font file \n");
			}
			if (text_frame == FRAME_TYPE_ASCII_ATTR_FONT || text_frame == FRAME_TYPE_ASCII_ATTR )
			{
				formatStringWithAttr(message);
			}
			else
			{
				memset(formattedString, 0, TEXT_LEN);
				// Format the text and paint it
...
time_t getLastPacketTime (SOCKET fd)
{
	...
	{
		if (pthread_mutex_lock(&vnc_session_guard[i]) == 0)
		{
			if (sessionInfo[i].sock == fd)
			{
				lastPacketTime = sessionInfo[i].lastPacketTime;

				if (pthread_mutex_unlock(&vnc_session_guard[i]) != 0)
	...
	for(i = 0; i < vnc_max_session; i++)
	{
		if (pthread_mutex_lock(&vnc_session_guard[i]) == 0)
		{
			if (sessionInfo[i].sock == fd)
			{
				...
				{
					sessionInfo[i].status = ACTIVE; /* Fortify [Dead Code]:: False Positive */
				}
				status = 0;
			...
			if (sessionInfo[i].sock <= 0)
			{
				...
				if (session_management == ENABLED)
				{
					if (clientIP != NULL)
					{
						...
						/* snprintf() will modify existing value. So need to update the variable accordingly post snprintf() execution. */
...
int formatStringWithAttr (char* src)
{
	int index, charLen, len, textWidth,textHeight;
	int screen_x = 0;
	int screen_y = SCREEN_POS_Y;
	int attribute = -1;
	int fg_color_index,bg_color_index;
	charLen = fg_color_index = bg_color_index = 0;
	...
	for (index = 0; index < len; index++)
	{
		...
		if(charLen == textWidth)
		{
			charLen = 0;
			screen_x =0;
			screen_y= screen_y+ SCREEN_POS_Y;
		}
		if( index % 2 == 0 ){
			// No need to Skip the NON_PRINT_CHAR_RANGE char because alt key code have some special ascii characters.
			attribute = src[index+1];
			fg_color_index = (attribute & 0xF);
			bg_color_index = (attribute & 0xF0) >> 4;
			screen_x+=rfbDrawCharWithClip(rfbScreen,(curFont ? curFont: &radonFont),screen_x ,screen_y,src[index],0,0,maxx,maxy,color[fg_color_index],color[bg_color_index]);
			charLen++;
		}
	}
	return 1;
...
int formatString (char* src,char * formattedString )
{
	...
	charLen = aIndex = screen_x = 0;
	screen_y = SCREEN_POS_Y;
	// Fix for Fortify [Poor Style: Value Never Read]
	textWidth = (frame_hdr_vnc.res_x);
	textHeight = ( frame_hdr_vnc.res_y  / frame_hdr_vnc.params.char_height);
	len =  (textWidth * textHeight * 2);
	...
	for (index = 0; index < len; index++)
	{
		...
		aIndex++;
#if defined(SOC_PILOT_III) || defined(SOC_PILOT_IV)
		screen_x+=rfbDrawChar(rfbScreen,(curFont ? curFont : &radonFont),screen_x,screen_y,src[index],0xFFFFFF);
#else
		screen_x+=rfbDrawChar(rfbScreen,&radonFont,screen_x,screen_y,src[index],0xFFFFFF);
#endif
	}
	return 1;
}
...
int isCleanUpSession(SOCKET fd)
{
        ...
        if(session_management != ENABLED){
        	...
        	if (fd > 0)
        	{
        		while(sessionIndex--)
        		{
					if (pthread_mutex_lock(&vnc_session_guard[sessionIndex]) == 0)
					{
						if (sessionInfo[sessionIndex].sock == fd)
						{
							...
							if (pthread_mutex_unlock(&vnc_session_guard[sessionIndex]) != 0)
								printf("[%s:%d] pthread_mutex_unlock failed : %d\n", __FUNCTION__, __LINE__, errno);
	...
	int sess_index = -1;
		...
		else
		{
			if(active_sock_list != NULL){
				...
				{
					if (pthread_mutex_lock(&vnc_session_guard[sess_index]) == 0)
					{
						...
						if (sock_index >= actvSessCount)
						{
							...
							if (pthread_mutex_unlock(&vnc_session_guard[sess_index]) != 0)
								printf("[%s:%d] pthread_mutex_unlock failed : %d\n", __FUNCTION__, __LINE__, errno);
...
{	// vnc_max_session, sessionInfo values are initialized upon receiving RECV_VNC_CONF command in pipe
	if (NULL != (sessionInfo = (VNCSessionInfo *) malloc ((sizeof(VNCSessionInfo) * vnc_max_session))) ) // Fix for Fortify [Missing Check against Null] 
...
{	// vnc_max_session value is initialized upon receiving RECV_VNC_CONF command in pipe
	if (NULL == (vnc_session_guard = (pthread_mutex_t *)calloc(vnc_max_session, sizeof(pthread_mutex_t))) ) /* Fortify [Intger Overflow]:: False Positive */ /* vnc_max_session is depending on vnc_max count which is in limited range - Reason */
		printf("[%s:%d] vnc_session_guard is NULL\n", __FUNCTION__, __LINE__); // Fix for Fortify [Missing Check against Null]
...
int openVncPipe(char *pipename) {
	...
	if (pipename != NULL) {
		...
		fd = open(pipename, O_RDWR|O_NONBLOCK); /* Fortify [Race Condition: File System Access]:: False Positive */ /* The file does not change between the calls of unlink() and open() - Reason*/
		if (fd < 0) {
			...
			switch (cmd.cmd)
			{
				case KVM_CLIENT_STATE:
					if( ( cmd.status == ADVISER ) || ( cmd.status == VNC ) ) {
						...
						if( DisableAutoSwitch == 1 ) { // Will be true if respective hidden feature is enabled in P.CONFIG
							if( g_kvm_client_state != VNC ) { /* Fortify [Dead Code]:: False Positive */
								vncUnloadResource();
								onStartStopVnc(STOP);
							}
						}
...
#endif
#ifdef CONFIG_SPX_FEATURE_VNC_PASSWORD_FOR_NON_SECURE_CONNECTION /* Fortify [Password Management: Password in Comment]:: False Positive */
...
int loadDefaultVNC()
{
  ...
  if((rfbScreen->frameBuffer = (char*)malloc(maxx*maxy*bpp)) == NULL) // Fix for Fortify [Missing Check against Null]
  {
